...
const getInputId = (id: InputId) => `${id}-password`;
...
export const LoginForm = () => {
  const onSubmit: FormEventHandler<HTMLFormElement> = (e) => {
    ...
    const id = target.id as (typeof formIds)[number];

    const inputId = getInputId(id);
    const input = target.elements.namedItem(inputId) as HTMLInputElement;
    const password = input.value;

    apiClient.auth.login.post({ name: id, password }, { fetch: { credentials: "include" } });
  };
